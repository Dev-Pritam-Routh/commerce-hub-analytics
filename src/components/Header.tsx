...
const Header = () => {
  ...
  return (
    <header
      className={cn(
        "fixed top-4 left-0 right-0 z-50 transition-all duration-300 py-3 mx-auto",
        isScrolled 
          ? "max-w-[95%] lg:max-w-[90%] px-6" 
          : "max-w-full px-6 md:px-10",
        isScrolled
          ? "bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg shadow-gold-sm dark:shadow-gold rounded-full"
          : "bg-transparent"
      )}
    >
      <div className="flex items-center justify-between">
        ...
        <Link 
          to="/" 
          className="text-2xl font-bold font-serif tracking-tight text-primary dark:text-primary"
        >
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          {isAuthenticated && (
            <Link to="/orders" className="nav-link font-medium text-slate-900 dark:text-white">Orders</Link>
          )}
        </nav>
